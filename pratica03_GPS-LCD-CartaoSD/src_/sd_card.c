...
// Escrita no SD Card
void write_to_sd(double lat, double lon) {
    FRESULT fr;
    ...
    */
    ...
    if (fr == FR_OK) {
        ...
        // Formata string com coordenadas (5 casas decimais)
        char text[64];
        snprintf(text, sizeof(text), "Lat: %.5f, Lon: %.5f\n", lat, lon);
