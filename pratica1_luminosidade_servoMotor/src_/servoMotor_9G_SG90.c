...
// ---- Ajusta o servo conforme luminosidade medida ----
void ajustar_servo(uint16_t value_lux)
{
    uint32_t duty_us;

    if (value_lux <= 100) {
        duty_us = DUTY_MIN; // pouca luz -> giro horário rápido
        printf("Servo: Giro horário rápido - Duty: %uus\n", duty_us);
    }
    else if (value_lux <= 500) {
        duty_us = DUTY_MID; // luz média -> parado
        printf("Servo: Parado - Duty: %uus\n", duty_us);
    }
    else {
        duty_us = DUTY_MAX; // muita luz -> giro anti-horário rápido
        printf("Servo: Giro anti-horário rápido - Duty: %uus\n", duty_us);
    }
